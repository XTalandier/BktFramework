<?php
class Bkt_Ui_Pagination {
	
	public static $boutonSuivant   = '&gt;&gt;';
	public static $boutonPrecedent = '&lt;&lt;';
	
	public static function getInstance($url, $param, $total, $current = 1, $adj=3){
		$pagination = new Bkt_Ui_Pagination();
		return $pagination->paginate($url, $param, $total, $current, $adj=3);
	}
	 ...
	 */
	public function paginate($url, $param, $total, $current = 1, $adj=3) {
		...
		if ($total > 1) {
			...
			/* ////////// Début affichage du bouton [précédent] ////////// */
			 // la page courante est la 2, le bouton renvoit donc sur la page 1, remarquez qu'il est inutile de mettre ?p=1
			if ($current == 2){
				$pagination .= "<a href=\"{$url}\" class=\"precedent\">".self::$boutonPrecedent."</a>";
				...
				$pagination .= "<a href=\"{$url}{$param}{$prev}\" class=\"precedent\">".self::$boutonPrecedent."</a>";
			}else{ // dans tous les autres, cas la page est 1 : désactivation du bouton [précédent]
				//$pagination .= '<span class="inactive">'.self::$boutonPrecedent.'</span>';
			}
		}
		 ...
		 */
			...
			*/
			...
			if ($current == $total){
				//$pagination .= "<span class=\"inactive\" class=\"suivant\">".self::$boutonSuivant."</span>\n";
			}else{
				$pagination .= "<a href=\"{$url}{$param}{$next}\" class=\"suivant\">".self::$boutonSuivant."</a>\n";
			}
